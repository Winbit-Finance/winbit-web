...
export const HistoryPage = () => {
  ...
  const statusPillClass = (status) => {
    ...
    // Completed / Acreditado / Aprobado (institutional green)
  ...
  const isRejectedOrPending = (status) => {
    const s = normalize(status);
    return s === 'rechazado' || s === 'rejected' || s === 'pendiente' || s === 'pending';
  };

  const isTradingCommissionMovement = (movement) => {
    const m = normalize(movement);
    return m === 'trading_fee' || m === 'trading_fee_adjustment';
  };

  const isWithdrawalMovement = (movement) => {
    const m = normalize(movement);
    return m === 'retiro' || m === 'withdrawal' || m === 'deposit_reversal';
  };

  const amountClass = (row) => {
    // Default: neutral operational movement (white)
    if (isRejectedOrPending(row?.status)) return 'text-text-primary';

    // Trading commissions must stay neutral (white, not red)
    if (isTradingCommissionMovement(row?.movement)) return 'text-text-primary';

    // Withdrawals stay neutral (white)
    if (isWithdrawalMovement(row?.movement)) return 'text-text-primary';
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 pb-2 border-b border-[rgba(101,167,165,0.15)] text-text-primary tracking-wide">
          {t('history.subtitle')}
        </p>
      </div>
