...
const DASHBOARD_BALANCE_HIDDEN_KEY = 'winbit-dashboard-hide-balances';
...
export const DashboardPage = () => {
  ...
  const [isBalanceHidden, setIsBalanceHidden] = useState(() => {
    ...
    try {
      return localStorage.getItem(DASHBOARD_BALANCE_HIDDEN_KEY) === 'true';
    } catch {
      return false;
    }
  });
  ...
  useEffect(() => {
    try {
      localStorage.setItem(DASHBOARD_BALANCE_HIDDEN_KEY, String(isBalanceHidden));
    } catch {
  ...
  return (
    <div className="space-y-8">
      ...
      {/* KPI summary */}
      <div>
        {/* Main KPI: standalone block (no card container) */}
        <div className="w-full">
          <div className="flex items-start justify-between gap-3">
            <div className="min-w-0 flex-1">
              <div className="text-sm text-[#8dc8bf]">{t('dashboard.kpis.currentValue')}</div>
              <div className="mt-2 text-4xl md:text-5xl font-semibold tracking-tight text-text-primary">
                {isBalanceHidden ? HIDDEN_BALANCE_PLACEHOLDER : formatCurrency(data.balance)}
              </div>
            </div>
            <button
              type="button"
              onClick={() => setIsBalanceHidden((v) => !v)}
              className="shrink-0 rounded-lg border border-[rgba(255,255,255,0.08)] p-2 text-text-muted transition-colors hover:border-[rgba(101,167,165,0.35)] hover:text-[#8dc8bf]"
        ...
        <div className="mt-10 grid gap-6 md:grid-cols-2">
          <KpiCard
            ...
            variant="currency"
            tone="neutral"
            displayOverride={isBalanceHidden ? HIDDEN_BALANCE_PLACEHOLDER : undefined}
          />
          {/* Spacer to keep desktop KPI rows perfectly paired */}
